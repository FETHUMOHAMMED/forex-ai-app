"""
REAL FOREX AI TRAINER – Alpha Vantage (Fixed column mismatch)
"""
...
class RealForexTrainer:
    ...
    def fetch_data(self, pair, from_currency, to_currency='USD', outputsize='full'):
        """Fetch daily forex data – handle varying column counts"""
        print(f"📥 Fetching {pair} from Alpha Vantage...")
        fx = ForeignExchange(key=self.api_key)
        try:
            raw_data, meta = fx.get_currency_exchange_daily(
        ...
        if not isinstance(raw_data, dict):
            print(f"   Unexpected response: {type(raw_data)}")
            return None
        ...
        # Alpha Vantage returns either 4 or 5 columns (volume may be missing)
    ...
    def train_for_pair(self, pair, from_curr, to_curr='USD'):
        ...
        if df is None or len(df) < 500:
            print(f"❌ Insufficient data for {pair} (got {len(df) if df is not None else 0} rows)")
            return None
    ...
    def run(self):
        print("="*60)
        print("REAL FOREX AI TRAINER – Alpha Vantage (Fixed)")
        print("="*60)
        results = {}
