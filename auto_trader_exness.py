...
class AutoTrader:
    def __init__(self):
        ...
        for acc_cfg in accounts_cfg:
            state = AccountState(acc_cfg, CONFIG, self.ai, self.logger,
    ...
    def daily_monte_carlo_check(self):
        ...
        pnls = self.logger.get_recent_pnls(limit=100)
        if len(pnls) < 30:
    ...
    def manage_account(self, acc):
        ...
        for pos in positions:
            try:
                pair = pos['symbol']
                self.apply_time_exit(acc, pair, pos, max_hours=24)
            except Exception as e:
        ...
        for pair in acc.pairs:
            ...
            # Get signal – REAL ONLY, no mock fallback
            acc.ai.strict_mode = (acc.name != 'Demo2')
            print(f"[FETCH DEBUG] Calling get_real_signal for {pair}")
            signal = acc.ai.get_real_signal(pair)
            ...
            # Regime filter – Demo2 collects all regimes; other accounts stay protected
            if acc.name != 'Demo2' and regime not in ('volatile', 'ranging'):
                logger.info(f"REJECTED {pair}: regime={regime} not allowed (acc={acc.name})")
                self.rejection_counts['volatile_skip'] += 1
                continue

            MAJORS = {'EURUSD','GBPUSD','USDJPY','AUDUSD','USDCAD','USDCHF','NZDUSD','USDSGD'}

            if pair in MAJORS:
                atr_min_nj = getattr(acc, 'atr_min_non_jpy', self.atr_min_non_jpy)
                atr_max_nj = getattr(acc, 'atr_max_non_jpy', self.atr_max_non_jpy)
                atr_min_j  = getattr(acc, 'atr_min_jpy', self.atr_min_jpy)
                atr_max_j  = getattr(acc, 'atr_max_jpy', self.atr_max_jpy)

                regime_default = {
                    'min_confidence': acc.min_confidence,
                    'risk_percent': acc.risk_percent,
                    'atr_min_non_jpy': atr_min_nj,
                    'atr_max_non_jpy': atr_max_nj,
                    'atr_min_jpy': atr_min_j,
                    'atr_max_jpy': atr_max_j,
                    'sl_atr_mult': 1.5,
                    'tp_atr_mult': 2.5
                }
                if isinstance(acc.regime_params, dict) and pair in acc.regime_params:
                    params = acc.regime_params[pair].get(regime, regime_default)
                elif isinstance(acc.regime_params, dict) and 'global' in acc.regime_params:
                    params = acc.regime_params['global'].get(regime, regime_default)
                else:
                    params = acc.regime_params.get(regime, regime_default)
            else:
                vol_params = acc.regime_params.get('volatile', {})
                params = {
                    'min_confidence': vol_params.get('min_confidence', 0.56),
                    'risk_percent': vol_params.get('risk_percent', 0.25),
                    'atr_min_non_jpy': vol_params.get('atr_min_non_jpy', 0.001),
                    'atr_max_non_jpy': vol_params.get('atr_max_non_jpy', 0.008),
                    'atr_min_jpy': vol_params.get('atr_min_jpy', 0.08),
                    'atr_max_jpy': vol_params.get('atr_max_jpy', 0.45),
                    'sl_atr_mult': vol_params.get('sl_atr_mult', 3.0),
                    'tp_atr_mult': vol_params.get('tp_atr_mult', 4.0)
                }

            min_conf = params.get('min_confidence', acc.min_confidence)

            if acc.name == 'Demo2':
                effective_min_conf = 0.50   # collect everything
            else:
            ...
            if tick is not None:
                ...
                if (spread > max_spread or
                    (pair in ('EURUSD','GBPUSD','AUDUSD','NZDUSD') and spread > 0.0003) or
                    (pair in ('USDJPY','USDCAD','USDCHF','USDSGD') and spread > 0.0004)):
            ...
            if quality < 5:   # requires at least 5 out of 8 points
                logger.info(f"REJECTED {pair}: trade quality score {quality}/8 too low")
                continue
            ...
            order_result = acc.broker.place_market_order(
                pair, direction, entry_price, sl, tp,
