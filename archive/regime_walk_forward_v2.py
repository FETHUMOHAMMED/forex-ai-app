"""
Regime Walk-Forward V2 - Tests DISTRIBUTING+BREAKOUT edge across periods.
"""
...
for i in range(200, len(df)-20, 10):
    ...
    try:
        ...
        if sig.is_valid:
            # Apply regime filter: DISTRIBUTING + BREAKOUT
            if sig.dealer_pressure == 'DISTRIBUTING' and sig.institutional_bias == 'BREAKOUT' and sig.confidence >= 0.55:
...
total = len(signals)
print(f"Total DISTRIBUTING+BREAKOUT signals: {total}")

if total < 30:
    print("Not enough signals for walk-forward")
    exit()
...
print(f"\n{'='*55}")
print(f"  REGIME WALK-FORWARD: DISTRIBUTING + BREAKOUT")
print(f"  EURUSD | Conf >= 0.55")
print(f"{'='*55}")
...
if all_pf:
    ...
    all_ok = all(pf > 1.15 for pf in all_pf)
    print(f"\n  {'='*40}")
    print(f"  Avg PF: {avg_pf:.2f}")
    print(f"  Consistency: {'PASS - EDGE VERIFIED' if all_ok else 'FAIL'}")
    print(f"  {'='*40}")
print(f"\n{'='*55}")
